# from LowerBound_new import LowerBound
...
class NetworkFlowModelMulti():
    ...
    def get_solutions(self):
        ...
        for i in range(self.n_solutions):
            # print('i',i)
            # self.v_used_arc.setAttr("Xn", i)
            # self.v_flow.setAttr("Xn", i)
            # self.v_used_arc.xn = i
            # self.v_flow.xn = i

            # self.v_used_arc.solutionNumber = i
            # self.v_flow.solutionNumber = i

            self.m.Params.SolutionNumber = i

            # self.m.solutionNumber = i
    ...
    def get_stack_indices_for_each_lane(self):
        ...
        for edge in self.cost:
            if self.v_used_arc[edge].xn >= (1 - 1e-09) and self.v_flow[edge].xn >= (1 - 1e-09) and edge[
    ...
    def run_model(self):
        self.m.setParam('Seed', 0)
        self.m.setParam("OutputFlag", False)
        self.m.setParam("PoolSearchMode", 2)  # mode 2
        self.m.setParam("PoolSolutions", 10)  # 10 best solutions
        self.m.setParam("PoolGap", 0)  # ensures only optimal solutions
        self.m.optimize()
        self.n_solutions = self.m.getAttr("SolCount")
